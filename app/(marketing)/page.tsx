...
function BeforeAfterIllustration({ t }: { t: (key: string) => string }) {
  ...
  const tones = [
    { label: t('dashboard.tone_professional'), cls: 'bg-blue-500/15 text-blue-400 border-blue-500/25' },
    { label: t('dashboard.tone_friendly'), cls: 'bg-green-500/15 text-green-400 border-green-500/25' },
    { label: t('dashboard.tone_direct'), cls: 'bg-amber-500/15 text-amber-400 border-amber-500/25' },
  ]
    ...
    <div ref={ref} className="max-w-5xl mx-auto w-full">
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <motion.div
          ...
          className="relative rounded-2xl overflow-hidden border border-red-500/20 bg-card"
        >
          {/* Header bar */}
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-red-500/15 bg-red-500/5">
            ...
            <div className="flex items-center gap-1.5 text-red-400/60">
              ...
              <span className="text-xs font-mono">bez AI</span>
            </div>
          </div>
          ...
          <div className="p-5 space-y-3">
            ...
            >
              <div className="h-8 w-8 rounded-full bg-slate-500 shrink-0 flex items-center justify-center text-[10px] font-bold text-white shadow-sm">K</div>
              <div className="rounded-2xl rounded-bl-sm bg-muted px-3.5 py-2.5 text-xs leading-relaxed max-w-[82%] shadow-sm">
                {t('landing.hero_demo_message')}
                <p className="text-[9px] text-muted-foreground/50 mt-1">09:14</p>
              </div>
            </motion.div>
            ...
            >
              <div className="h-8 w-8 rounded-full bg-slate-600/60 shrink-0 flex items-center justify-center text-[10px] font-bold text-white/50 shadow-sm">A</div>
              <div className="rounded-2xl rounded-br-sm bg-muted/50 border border-border/40 px-3.5 py-2.5 text-xs text-muted-foreground/60 italic max-w-[82%]">
                {t('landing.before_thinking')}
                <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ duration: 1.3, repeat: Infinity }}>...</motion.span>
              </div>
            </motion.div>
            ...
            >
              <div className="h-8 w-8 rounded-full bg-slate-600/60 shrink-0 flex items-center justify-center text-[10px] font-bold text-white/50 shadow-sm">A</div>
              <div className="rounded-2xl rounded-br-sm bg-muted/50 border border-border/40 px-4 py-3.5">
                <div className="flex gap-1.5">
                  {[0, 0.18, 0.36].map((d, i) => (
                    <motion.div key={i} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.75, repeat: Infinity, delay: d }}
          ...
          <motion.div
            ...
            className="mx-5 mb-5 rounded-xl bg-red-500/8 border border-red-500/20 py-4 flex flex-col items-center gap-1"
          >
            <motion.span
              animate={inView ? { opacity: [1, 0.35, 1] } : {}}
              transition={{ duration: 1.4, repeat: Infinity, delay: 1.4 }}
              className="text-5xl font-black tabular-nums text-red-400 tracking-tight leading-none"
            >
              8:47
            </motion.span>
            <p className="text-xs font-semibold text-red-400/70 mt-1">{t('landing.before_caption')}</p>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className="relative rounded-2xl overflow-hidden border border-primary/25 bg-card shadow-lg shadow-primary/5"
        >
          {/* Glow top */}
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-primary/15 bg-primary/8">
            ...
            <div className="flex items-center gap-1.5 text-primary/60">
              ...
              <span className="text-xs font-mono">ReplyPro AI</span>
            </div>
          </div>
          ...
          <div className="p-5 space-y-3">
            ...
            <motion.div
              ...
              transition={{ delay: 0.85, duration: 0.45, type: 'spring', stiffness: 320 }}
              className="flex gap-2.5 items-end flex-row-reverse"
            >
              <div className="h-8 w-8 rounded-full bg-primary shrink-0 flex items-center justify-center shadow-md shadow-primary/35">
                <Sparkles className="h-3.5 w-3.5 text-primary-foreground" />
              </div>
              <div className="rounded-2xl rounded-br-sm bg-primary/12 border border-primary/20 px-3.5 py-2.5 text-xs leading-relaxed max-w-[82%] shadow-sm">
            ...
            <motion.div
              ...
              className="flex gap-1.5 flex-wrap pl-10"
            >
              {tones.map((b, i) => (
                <motion.span
                  key={b.label}
                  ...
                  animate={inView ? { opacity: 1, scale: 1 } : {}}
                  transition={{ delay: 1.25 + i * 0.1, type: 'spring', stiffness: 380 }}
                  className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-semibold ${b.cls}`}
                >
                  {b.label}
                </motion.span>
              ))}
          ...
          <motion.div
            ...
            className="mx-5 mb-5 rounded-xl bg-primary/8 border border-primary/20 py-4 flex flex-col items-center gap-1"
          >
            <span className="text-5xl font-black tabular-nums text-primary tracking-tight leading-none">0:05</span>
            <p className="text-xs font-semibold text-primary/70 mt-1">{t('landing.after_caption')}</p>
          </motion.div>
        </motion.div>
      </div>
      ...
      >
        <span className="font-bold text-red-400 tabular-nums text-base">8:47</span>
        <div className="flex items-center gap-1 text-muted-foreground/40">
          <div className="h-px w-8 bg-current" />
          <ArrowRight className="h-3.5 w-3.5" />
          <div className="h-px w-8 bg-current" />
        </div>
        <span className="font-bold text-primary tabular-nums text-base">0:05</span>
        <span className="rounded-full bg-primary/10 border border-primary/20 px-3 py-1 text-xs font-bold text-primary ml-1">
